...
static _Atomic(value) global_scan_ptr_atomic;
...
value copy(value v){
    ...
    while(1){
        ...
        if (header == 0) {
            // already forwarded
            // Relaxed is fine here because 'acquire' on header already established visibility
...
void* cheney_bfs(void* arg) {
    (void)arg;
    const value BATCH_WORDS = 64; // Claim up to 64 words at a time

    for (;;) {
        // Read the global scan and free limits
        value current_scan = atomic_load_explicit(&global_scan_ptr_atomic, memory_order_acquire);
        value current_free = atomic_load_explicit(&free_ptr_atomic, memory_order_acquire);

        // If we've caught up to the free pointer
        if (current_scan >= current_free) {
            // Verify no other thread is mid-copy, which would bump free_ptr again
            if (atomic_load_explicit(&gc_copy_in_progress, memory_order_acquire) == 0) {
                // Double check free_ptr hasn't moved after our copy_in_progress check
                if (current_scan >= atomic_load_explicit(&free_ptr_atomic, memory_order_acquire)) {
                    return NULL;
                }
            }
            // Wait for more objects to land in to_space
            #if defined(__x86_64__) || defined(__i386__)
            __builtin_ia32_pause(); 
            #endif
            continue;
        ...
        value* scan_start = to_heap + current_scan;
        value batch_size = 0;

        // Peek ahead to find object boundaries to form a batch
        while (current_scan + batch_size < current_free && batch_size < BATCH_WORDS) {
            value h = atomic_load_explicit((_Atomic(value)*)(scan_start + batch_size), memory_order_relaxed);
            
            // Handle race condition: free_ptr bumped, but header is mid-write (0) or claimed (-1)
            if (h == 0 || h == -1) break; 
            
            // Handle race condition: Header is written, but object is mid-copy 
            if (h & GC_HDR_COPYING) break;

            value base = h & ~GC_HDR_PHYS_MASK;
            long tot = (long)(base >> 10);
            if (tot <= 0) break; // Incomplete object write, stop batch here
            
            batch_size += tot; // Advance strictly by exact object boundaries
        }

        if (batch_size == 0) {
            // Memory is claimed by free_ptr but headers aren't fully baked yet. Yield and retry.
            #if defined(__x86_64__) || defined(__i386__)
            __builtin_ia32_pause(); 
            #endif
            continue; 
        ...
        // Claim the entire batch with ONE atomic operation!
        if (atomic_compare_exchange_weak_explicit(&global_scan_ptr_atomic, &current_scan, current_scan + batch_size, memory_order_acquire, memory_order_relaxed)) {
            
            // SUCCESS! We exclusively own [scan_start, scan_start + batch_size).
            // We can scan this memory normally. No header CAS needed!
            value processed = 0;
            while (processed < batch_size) {
                value* obj = scan_start + processed;
                value h = *obj; // Standard read, no atomic overhead
...
void* semi_space_collection(void*){
    ...
    atomic_store(&global_scan_ptr_atomic, 0); // Initialize new batch pointer
